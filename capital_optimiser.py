...
# -----------------------------------------------------------------------------
# How to run (locally)
# 1) python -m pip install streamlit pulp pandas highspy
# 2) python -m streamlit run capital_optimiser.py
#
...
# Defaults (also shown in Advanced JSON editor)
DEFAULT_JSON = """{
...
def is_binding(used: float, cap: float, rel: float = 0.005) -> bool:
    """Return True if the utilised amount is within `rel` (0.5%) of the cap."""
...
def pick_solver():
    """
    Prefer HiGHS (from highspy). Fall back to CBC if available.
    Return a pulp solver instance or None if nothing is available.
    """
    # Try HiGHS
    try:
        hs_cls = getattr(pulp, "HiGHS_CMD", None)
        if hs_cls is not None:
            s = hs_cls(msg=False)
...
def build_and_solve(
    ...
    *,
    ...
    Returns: (status, obj, df_results, totals_dict, bindings_list)
    """
    capital = cfg["capital"]
    ...
    if solver is None:
        ...
        return "NoSolver", None, None, totals, []
...
# Effective config from quick inputs
effective_cfg = {
    "capital": {
        "CET1": q_CET1,
...
if solve_clicked:
    ...
    if status == "NoSolver":
        st.error("No LP solver found. On Streamlit Cloud, ensure `highspy` is in requirements.txt. Locally, install with `pip install highspy`.")
        st.stop()
